    ...
    public void run() {
        ...
        try {
            ...
            if (length == null || length.isEmpty()) {
                Log.e(Constants.TAG, "[COMMUNICATION THREAD] Error receiving parameters from client (length)!");
                return;
            }
            ...
            if (data.containsKey(word)) {
                ...
                params.add(new BasicNameValuePair("minLetters", length));
                UrlEncodedFormEntity urlEncodedFormEntity = new UrlEncodedFormEntity(params, HTTP.UTF_8);
                httpPost.setEntity(urlEncodedFormEntity);
                ResponseHandler<String> responseHandler = new BasicResponseHandler();
                String pageSourceCode = httpClient.execute(httpPost, responseHandler);

//                String url = "http://services.aonaware.com/CountCheatService/CountCheatService.asmx/LetterSolutionsMin?anagram=";
//                url = url + word + "&" + "minLetters=" + length;
...
//                String pageSourceCode = httpClient.execute(httpGet, responseHandlerGet);
                ...
                Log.d(Constants.TAG, "Test" + pageSourceCode);
                Log.d(Constants.TAG, "Test2" + element.data());

                information = "";

                StringBuilder copyPage = new StringBuilder(pageSourceCode);
                int startIndex = copyPage.indexOf("<string>", 0);
                int endIndex = copyPage.indexOf("</string>", 0);
                while (startIndex >= 0 && endIndex >= 0) {
                    String current_word = pageSourceCode.substring(startIndex + "<string>".length(), endIndex);
                    information = information + " " + current_word;

                    startIndex = copyPage.indexOf("<string>", startIndex + 1);
                    endIndex = copyPage.indexOf("</string>", endIndex + 1);
                }
...
//                    information = "NO INFO";
//                } else {
//                    information = results.getJSONObject(0).toString();
//                }



//                Elements elements = element.getElementsByTag("body");
//                Log.d(Constants.TAG, "Len " + elements.last().data());
//                for (Element script: elements) {
//                    String scriptData = script.data();
//                    Log.i(Constants.TAG, script.data());
